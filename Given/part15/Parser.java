public class Parser {
	...
	gcprint("fprintf(stderr, \"subscript (%d) out of "
			+"bounds for array %s[%d:%d] on line %d\\n\", "
			+"exp, s, lb, ub, lno);");
	gcprint("exit(1);");
	gcprint("} else {");
	gcprint("return exp;");
	...
	private void every(){
		...
		mustbe(TK.EVERY);
		if(is(TK.ELEMENT)||is(TK.INDEX)){
			if(is(TK.INDEX))
				isElement = false;
			scan();
		}
		if(is(TK.FORWARD)||is(TK.REVERSE)){
			if(is(TK.REVERSE))
				isForward = false;
			scan();
		}
		if(!is(TK.ID))//index variable
			parse_error("every expects id1.");
		symtab.begin_st_block();
		symtab.add_entry(tok.string, tok.lineNumber, TK.VAR);//add ID1 to new block
        Entry f = symtab.search(tok.string);
        f.setIsIV(true);
		
        Token id1 = tok;
		scan();
		mustbe(TK.COLON);
		if(!is(TK.ID)) //declared array
			parse_error("every expects id2.");
        Token id2 = tok;
		Entry e = symtab.search(tok.string);
		if(e == null || !e.isArray())
			parse_error("right-hand-side of : in every isn't array e");
		scan();
		mustbe(TK.DO);
		//can make code here? NOT YET
		int lb = e.getArrayOffset();
		int ub = lb + e.getArraySize() - 1;
        gcprint("{");
        gcprint("int ");
        gcprintid(id1.string);
        gcprint(";");
        
        gcprint("int i;");        
        
		if(isForward){//do forward or backward....... >_<!
            gcprint("for(i="+lb+"; i<="+ub+"; i++){");
//        gcprint("for(");
//        gcprintid(id1.string);
//        gcprint("="+lb+"; ");
//        gcprintid(id1.string);
...
//        gcprint(") {");
        ...
        gcprint("x_" + id1.string + " = ");
        if (isElement)
            gcprint("x_" + id2.string + "[i-("+lb+")];");
        else
            gcprint("i;");
		block();
		gcprint("}}");

...
// QUESTION 3:  What does the following case and the code in it do?
    ...
    private TK keywordLookup(String str) {
        ...
        if (str.equals("elsif"))     return TK.ELSIF;
        if (str.equals("else"))      return TK.ELSE;
        if (str.equals("end"))       return TK.END;
        ...
        if (str.equals("element"))        return TK.ELEMENT;
        if (str.equals("index"))        return TK.INDEX;
        if (str.equals("forward"))        return TK.FORWARD;
        if (str.equals("reverse"))        return TK.REVERSE;
